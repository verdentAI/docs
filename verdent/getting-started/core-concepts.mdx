...
- Inside a project, you create a **Task** — an isolated work session where context accumulates as you iterate.
...
- When multiple tasks need to change code at the same time, you use **Workspaces** — isolated working environments that keep changes separated and reduce conflicts.
---
...
A **task** is a focused work session inside a project—think of it as a **chat session with llm**.
A task typically contains multiple rounds of conversation. Within a task, the llm's **context is shared and accumulated**; across different tasks, **context is isolated**.

This makes Tasks ideal for working on multiple threads in parallel without mixing discussions, assumptions, or decisions.
...
| Action | Description |
|--------|-------------|
| **New Task** | Start a new isolated session for a new thread of work |
...
| --------- | --------------------- | ------- | -------- |
| Anthropic | claude-opus-4.5       | 200K    | Great for deep coding tasks and problem-solving. |
| Anthropic | claude-sonnet-4.5     | 200K    | Balanced complex tasks |
| Anthropic | claude-haiku-4.5      | 200K    | Fast, simple tasks |
| Google    | gemini-3-pro-preview  | 1M      | Advanced reasoning and multimodal |
| Google    | gemini-3-flash-preview| 1M      | Pro-level reasoning with Flash-level latency and efficiency |
| OpenAI    | gpt-5                 | 272K    | Reasoning and creativity |
| OpenAI    | gpt-5-codex (Beta)    | 272K    | Coding and debugging |
| Zhipu AI  | GLM 4.7               | 200K    | Fast, simple tasks |
| Moonshot  | kimi-k2-turbo         | 262K    | Fast, simple tasks |
...
| **Sync from Base** | Bring the latest changes from the Base Workspace into a workspace |
| **Merge / Rebase to Base** | Integrate a workspace’s result back into the Base Workspace |
...
- **Task**: The work you're doing (conceptual)
- **workspace**: The isolated directory where the work happens (physical)

One workspace can support multiple tasks sequentially, but for parallel work, each task needs its own workspace.
</Accordion>

<Accordion title="Do tasks share context with each other?">
No. Context is **shared and accumulated within a single task** as the conversation continues. But **different tasks are isolated**—they don’t share context with each other.

This makes it easy to work on multiple threads without mixing assumptions, decisions, or discussion history.
</Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Workspace Isolation" icon="code-branch" href="/verdent/core-features/workspace-isolation">
    Git worktree architecture details
  </Card>
</CardGroup>
