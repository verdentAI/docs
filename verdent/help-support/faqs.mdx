...
- Same rule systems (user_rules.md, AGENTS.md)
</Accordion>
...
Click **Task Changes** in the middle panel to view all files changed within the current task.
</Accordion>
...
<Accordion title="How do I rebase changes from a worktree back to master?">
<Steps>
  ...
  <Step title="Rebase">
    Use **Workspace → Rebase to Master** to integrate changes
  </Step>
  <Step title="Clean Up">
...
<Accordion title="Can I use different MCP servers per project?">
Yes. Configure MCP servers in Settings → MCP for global availability, or use project-specific configuration.
</Accordion>
...
| `~/.verdent/user_rules.md` | Global user rules |
...
Rebase creates a cleaner, linear history that's easier to understand and bisect. Each feature branch is rebased onto master before integration.
</Accordion>
...
Open User Center from the top bar to view:
