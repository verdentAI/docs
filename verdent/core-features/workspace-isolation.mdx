...
Workspace Isolation ensures that changes from one task never affect another. Try different approaches in parallel, compare results, and selectively rebase only the changes you want back into master.
...
- When you want changes to go directly to master
...
When you're ready to integrate workspace changes back to master:
...
<Steps>
  ...
  <Step title="Rebase to Master">
    Click **Workspace Actions → Rebase to master** in the Workspace Bar
  </Step>
...
Use **Workspace Actions → Sync with master** to pull the latest changes from master into your workspace. This helps prevent large conflicts when rebasing.
...
| **Sync with master regularly** | Don't let workspaces diverge too far from master |
...
No direct conversion, but you can rebase all changes from the workspace to master, then delete the workspace. The branch history is preserved.
</Accordion>
